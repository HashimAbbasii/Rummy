...
public class JoinPrivateRoomHandler : MonoBehaviour
{
    ...
    public void OnJoinButtonClick()
    {
        ...
        {
            requestSent = true;
            DatabaseManager.Instance.JoinRoom(new JoinRoomData() {userId = userIDInputField.text, code = roomKeyInputField.text}, OnPrivateRoomJoinedComplete);
        }

        AudioManager.Instance.PlayAudio(AudioName.ButtonAudio);
    }

    private void OnPrivateRoomJoinedComplete(JoinRoomData joinRoomData, UnityWebRequest request)
    {
        if (request.downloadHandler.text.Contains("Invalid Code"))
        {
            requestSent = false;
        ...
        if (JsonConvert.DeserializeObject<Response>(request.downloadHandler.text).data.mode !=
            GameManager.Instance.gameMode.ToString())
        {
            DisplayMessage("Wrong Game Mode");
            requestSent = false;
            return;
        }
        ...
        if (request.result == UnityWebRequest.Result.Success)
        {
            ...
            NetworkManager.Instance.JoinPrivateRoom(GameManager.Instance.room.roomKey);
        }
        else
        {
            requestSent = false;
